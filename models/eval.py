...
def eval_normal_pixel_me(input, label):
    # input: h*w*ch
    # label: h*w*ch
    ...
    input_v = F.normalize(input,p=2)    
    label = label.contiguous().view(-1,ch)
    label_v = F.normalize(label,p=2)
    ...
    return mean, median, small, mid, large
